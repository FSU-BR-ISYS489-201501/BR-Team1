...
/*********************************************************************************************
  ...
  ********************************************************************************************/
	...
	$reviewersQuery =  "SELECT users.UserId, users.FName, users.LName FROM users INNER JOIN reviewers ON users.UserId=reviewers.UserId INNER JOIN reviewcis ON reviewers.ReviewerId=reviewcis.ReviewerId INNER JOIN criticalincidents ON criticalincidents.CriticalIncidentId=reviewcis.CriticalIncidentId WHERE reviewcis.CriticalIncidentId=$incidentId;";
	$reviewersIdQuery = "SELECT users.UserId FROM users INNER JOIN reviewers ON users.UserId=reviewers.UserId INNER JOIN reviewcis ON reviewers.ReviewerId=reviewcis.ReviewerId INNER JOIN criticalincidents ON criticalincidents.CriticalIncidentId=reviewcis.CriticalIncidentId WHERE reviewcis.CriticalIncidentId=$incidentId;";
	...
	if(isset($_POST['submit'])){
		if(!empty($_POST['checkList'])) {		
			...
			foreach($_POST['checkList'] as $selected) {
				...
				$query = "DELETE FROM reviewcis WHERE ReviewerId=$reviewerId AND CriticalIncidentId=$incidentId;";
				//Run the query...
