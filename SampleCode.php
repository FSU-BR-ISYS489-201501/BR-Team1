<?php
	// Setting a varible and displaying its message
    $sampleVariable = "This is a variable!";
	echo($sampleVariable);
	
	// Giving a message to Software Developers
	$messageForDevelopers = "Write a comment on this page
		so that Mark knows that you understand how to 
		push to GitHub.";
	echo($messageForDevelopers);
	
	//Ben's comment to this page so that Mark knows I understand how to push to GitHub
	// Asking if Software Developers understand
	$understood = true;
	if ($understood = true) {
		echo("Understood!");
	}
	else {
		echo("I don't understand!");
	}
	
	// I am happy!
?>